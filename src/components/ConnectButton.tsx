...
export default function ConnectButton() {
  ...
  // const babyDodgeAddress = "0x84b9b910527ad5c03a9ca831909e21e236ea7b06";
  const babyDodgeAddress = "0xc748673057861a797275CD8A068AbB95A902e8de"
  ...
  const savePaymentHash = (hash: string, amount: string, mode: string) => {
    const result = {
      resultInfo: "payment",
      txnId: hash,
      bankTxnId: hash,
    ...
    axios
      .post("http://localhost:4000/api/v1/payment/save", {
        data: result,
      })
      .then((res) => {
        console.log(res);
  ...
  const sendBaby = useCallback(async () => {
    const web3 = new Web3(library.provider);
    const ctx = new web3.eth.Contract(abi as AbiItem[], babyDodgeAddress);

    const decimals = await ctx.methods.decimals().call();
    console.log("decimals : ", decimals);

    console.log(ctx.methods.transfer(recieverAdd, sendAmount));

    const result = await ctx.methods
    ...
    savePaymentHash(txHash, sendAmount.toString(), mode);
    onClose();
    valueload();
  ...
  const sendAction = useCallback(async () => {
    ...
    await web3.eth.sendTransaction(txParams, (error: any, hash: any) => {
      if (error) {
        ...
        savePaymentHash(hash, sendAmount.toString(), mode);
      ...
      <Modal isOpen={isOpen} onClose={onClose}>
        ...
        <ModalContent>
          ...
          <ModalFooter>
            ...
            <Button
              variant="ghost"
              onClick={mode === "BNB" ? sendAction : sendBaby}
            >
              Send
            </Button>
          </ModalFooter>
